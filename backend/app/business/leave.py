...
The one piece worth calling out: approve_leave() writes AttendanceStatus.ON_LEAVE
records for every day in the approved range. This isn't optional decoration —
it's the actual "Attendance -> Leave" link the SRS workflow diagram shows.
Without it, Attendance and Leave would be two disconnected modules that happen
to share an employee_id, and nothing downstream (Payroll, in Phase 3c) could
tell "present" apart from "on approved leave" by querying Attendance alone.
"""
...
# ⚠️ ASSUMPTION: SRS doesn't specify exact annual quotas. Adjust once real
# HR policy is confirmed. UNPAID has no entry -- it's uncapped by design.
LEAVE_ALLOCATIONS: dict[LeaveType, int] = {
    LeaveType.CASUAL: 12,
    LeaveType.SICK: 12,
    LeaveType.EARNED: 15,
}
...
def calculate_days(start_date: date_type, end_date: date_type) -> int:
    """Inclusive calendar-day count. Does not exclude weekends/holidays —
    the SRS doesn't define a work-week policy."""
    return (end_date - start_date).days + 1


def get_or_create_balance(db: Session, employee_id, leave_type: LeaveType, year: int) -> LeaveBalance:
    balance = db.query(LeaveBalance).filter(
        LeaveBalance.employee_id == employee_id, LeaveBalance.leave_type == leave_type, LeaveBalance.year == year
    ).first()
    if balance is None:
...
def apply_leave(db: Session, employee: Employee, leave_type: LeaveType, start_date: date_type, end_date: date_type, reason: str | None) -> LeaveRequest:
    ...
    request = LeaveRequest(
        employee_id=employee.id, leave_type=leave_type, start_date=start_date, end_date=end_date,
