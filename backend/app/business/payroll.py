...
def get_current_salary_structure(db: Session, employee_id, as_of: date_type) -> SalaryStructure:
    structure = (db.query(SalaryStructure).filter(SalaryStructure.employee_id == employee_id, SalaryStructure.effective_from <= as_of)
...
def _count_attendance_days(db: Session, employee_id, year: int, month: int) -> dict:
    ...
    records = db.query(Attendance).filter(Attendance.employee_id == employee_id, Attendance.date >= date_type(year, month, 1),
                                            Attendance.date <= date_type(year, month, days_in_month)).all()
...
def calculate_payslip(db: Session, employee: Employee, year: int, month: int, config: PayrollConfig) -> dict:
    ...
    return {"basic": basic, "hra": hra, "other_allowances": other_allowances, "overtime_amount": 0.0, "bonus_amount": 0.0,
            "gross_salary": gross_salary, "days_in_period": attendance["days_in_month"], "days_present": attendance["days_present"],
            "days_on_leave": attendance["days_on_leave"], "days_lop": attendance["days_lop"], "pf_deduction": pf_deduction,
            "esi_deduction": esi_deduction, "pt_deduction": pt_deduction, "tds_amount": 0.0, "net_salary": net_salary}
