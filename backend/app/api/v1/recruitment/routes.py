...
@router.post("/jobs", response_model=JobOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permission("recruitment:manage"))])
...
@router.patch("/jobs/{job_id}", response_model=JobOut, dependencies=[Depends(require_permission("recruitment:manage"))])
...
@router.post("/candidates", response_model=CandidateOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permission("recruitment:manage"))])
def create_candidate(payload: CandidateCreate, db: Session = Depends(get_db)):
    _get_job_or_404(db, payload.job_id)
...
@router.get("/candidates", response_model=list[CandidateOut], dependencies=[Depends(require_permission("recruitment:manage"))])
...
@router.get("/candidates/{candidate_id}", response_model=CandidateOut, dependencies=[Depends(require_permission("recruitment:manage"))])
...
@router.patch("/candidates/{candidate_id}/status", response_model=CandidateOut, dependencies=[Depends(require_permission("recruitment:manage"))])
def update_candidate_status(candidate_id: uuid.UUID, payload: CandidateStatusUpdate, db: Session = Depends(get_db)):
    candidate = _get_candidate_or_404(db, candidate_id)
    validate_transition(candidate.status, payload.status)
...
@router.post("/candidates/{candidate_id}/resume", response_model=CandidateOut, dependencies=[Depends(require_permission("recruitment:manage"))])
def upload_resume(candidate_id: uuid.UUID, file: UploadFile, db: Session = Depends(get_db), storage: StorageClient = Depends(get_storage_client)):
    candidate = _get_candidate_or_404(db, candidate_id)
...
@router.post("/candidates/{candidate_id}/screen", response_model=CandidateOut, dependencies=[Depends(require_permission("recruitment:manage"))])
def screen_candidate(candidate_id: uuid.UUID, db: Session = Depends(get_db), llm: LLMClient = Depends(get_llm_client), vector_store: VectorStore = Depends(get_vector_store)):
    candidate = _get_candidate_or_404(db, candidate_id)
    ...
    score, summary = parse_screening_response(raw_response)
...
@router.get("/jobs/{job_id}/similar-candidates", response_model=list[SimilarCandidateOut], dependencies=[Depends(require_permission("recruitment:manage"))])
def find_similar_candidates(job_id: uuid.UUID, limit: int = 5, db: Session = Depends(get_db), llm: LLMClient = Depends(get_llm_client), vector_store: VectorStore = Depends(get_vector_store)):
    job = _get_job_or_404(db, job_id)
