...
def check_in(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    record = record_check_in(db, employee)
...
def check_out(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    employee = _get_own_employee_record(db, current_user)
    record = record_check_out(db, employee)
    return AttendanceOut.model_validate(record)


@router.get("/me", response_model=list[AttendanceOut])
def get_my_attendance(start_date: date | None = None, end_date: date | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    employee = _get_own_employee_record(db, current_user)
...
def get_employee_attendance(employee_id: uuid.UUID, start_date: date | None = None, end_date: date | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
@router.post("/{employee_id}/mark", response_model=AttendanceOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permission("attendance:write"))])
