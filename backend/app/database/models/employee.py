...
Design: Employee is linked one-to-one with User (an employee needs to log
in) and optionally back to the Candidate they were hired from (traceability
through the SRS's Recruitment -> ... -> Employee pipeline, and a guard
against provisioning the same candidate twice — see candidate_id's unique
constraint).

Sensitive fields (Aadhaar, PAN, bank details) are stored per the SRS's
onboarding requirements ("Uploads Aadhaar, PAN, bank details..."). This is
product data for an HR system, not personal data about you the developer —
storing it here is the whole point of an HRMS. In a real production
deployment these would warrant column-level encryption; noted as a
...
class EmploymentStatus(str, enum.Enum):
    PENDING_ACTIVATION = "pending_activation"  # provisioned, but profile/docs/HR verification not complete
    ACTIVE = "active"
    INACTIVE = "inactive"
...
class Employee(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    # nullable: not every employee necessarily originated from the recruitment
    # pipeline (e.g. a legacy record entered directly by HR)
    ...
    employment_status: Mapped[EmploymentStatus] = mapped_column(
        Enum(EmploymentStatus), default=EmploymentStatus.PENDING_ACTIVATION, nullable=False
    )

    # --- Self-service profile fields (filled in during onboarding's "Profile Completion" step) ---
    ...
    # --- HR verification (per SRS: "HR verifies and activates employee account") ---
