...
def _provision(client, headers, candidate_id):
    return client.post("/api/v1/employees/provision", json={"candidate_id": candidate_id, "department": "Engineering", "designation": "Software Engineer", "date_of_joining": "2026-08-01"}, headers=headers)


def _complete_profile(client, employee_headers):
    return client.patch("/api/v1/employees/me", json={"phone": "9999999999", "address": "123 Main St", "date_of_birth": "1995-01-01", "bank_account_number": "123456"}, headers=employee_headers)
...
def test_change_password_wrong_current_password_rejected(client, db_session):
    ...
    login = client.post("/api/v1/auth/login", json={"email": "newhire@example.com", "password": temp_password})
    employee_headers = {"Authorization": f"Bearer {login.json()['access_token']}"}
    resp = client.post("/api/v1/auth/change-password", json={"current_password": "wrongpassword", "new_password": "brandnewpass123"}, headers=employee_headers)
...
def test_change_password_clears_flag_and_new_password_works(client, db_session):
    ...
    login = client.post("/api/v1/auth/login", json={"email": "newhire@example.com", "password": temp_password})
    employee_headers = {"Authorization": f"Bearer {login.json()['access_token']}"}
    resp = client.post("/api/v1/auth/change-password", json={"current_password": temp_password, "new_password": "brandnewpass123"}, headers=employee_headers)
...
def test_activation_requires_specific_required_document_types(client, db_session):
    ...
    try:
        ...
        client.post("/api/v1/auth/change-password", json={"current_password": temp_password, "new_password": "brandnewpass123"}, headers=employee_headers)
        ...
        def upload_doc(doc_type: str):
            ...
            buf = io.BytesIO()
            doc.save(buf)
            resp = client.post(f"/api/v1/employees/{employee_id}/documents?document_type={doc_type}", files={"file": (f"{doc_type}.docx", buf.getvalue(), "application/vnd.openxmlformats-officedocument.wordprocessingml.document")}, headers=employee_headers)
...
def test_onboarding_status_checklist_reflects_progress(client, db_session):
    ...
    assert body["ready_for_activation"] is False
