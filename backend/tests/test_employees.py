...
def _provision(client, headers, candidate_id, department="Engineering", designation="Software Engineer"):
    return client.post("/api/v1/employees/provision", json={"candidate_id": candidate_id, "department": department, "designation": designation, "date_of_joining": "2026-08-01"}, headers=headers)
...
def test_employee_can_view_and_update_own_profile(client, db_session):
    ...
    update = client.patch("/api/v1/employees/me", json={"phone": "9999999999", "address": "123 Main St", "date_of_birth": "1995-01-01", "bank_account_number": "123456"}, headers=employee_headers)
...
def test_hr_can_update_department_and_manager(client, db_session):
    ...
    resp = client.patch(f"/api/v1/employees/{employee_id}", json={"department": "Platform Engineering", "designation": "Senior Engineer"}, headers=headers)
    assert resp.status_code == 200
...
def test_full_onboarding_happy_path(client, db_session):
    ...
    try:
        ...
        client.patch("/api/v1/employees/me", json={"phone": "9999999999", "address": "123 Main St", "date_of_birth": "1995-01-01", "bank_account_number": "123456"}, headers=employee_headers)
        ...
        for doc_type in ["aadhaar", "pan", "bank_proof"]:
            ...
            buf = io.BytesIO()
            doc.save(buf)
            upload_resp = client.post(f"/api/v1/employees/{employee_id}/documents?document_type={doc_type}", files={"file": (f"{doc_type}.docx", buf.getvalue(), "application/vnd.openxmlformats-officedocument.wordprocessingml.document")}, headers=employee_headers)
